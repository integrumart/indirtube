...
class GlobalPlugin(globalPluginHandler.GlobalPlugin):
	...
	@globalPluginHandler.scriptHandler.script(
		description=_("Starts indirTube download dialog"),
		category=_("indirTube")
	)
	def script_indirTubeStart(self, gesture):
		...
		def show_dialog():
			choices = [
				_("MP3 (Audio)"), 
				_("MP4 (Video)"), 
				_("Donate (Support Developer)")
			]
			
			dlg = wx.SingleChoiceDialog(
				gui.mainFrame, 
				_("Select format:"), 
				_("indirTube v2.0 - Volkan Ozdemir"), 
				choices
			)
